    ...
    <main className="min-h-screen bg-[#0A0A0B] text-white light:text-gray-900 p-4 md:p-8 relative overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        {isLoading ? (
          ...
          <div className="text-center mt-32 text-white light:text-gray-900/50">
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="text-center mb-10">
              ...
              <p className="text-sm font-bold text-white light:text-gray-900/60">{data.challenge.description}</p>
            </div>

            <div className="glass-card p-6 border-white/5 rounded-[30px] bg-black/40 backdrop-blur-2xl">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-white light:text-gray-900/40 mb-6 border-b border-white/5 pb-4">Live Leaderboard</h3>
              
              <div className="space-y-3">
                {data.leaderboard.length === 0 ? (
                  <p className="text-center text-sm font-bold text-white light:text-gray-900/30 py-8">No data for this week yet.</p>
                ) : (
                  data.leaderboard.map((block: any, idx: number) => (
                    ...
                    >
                      <div className="flex items-center gap-4 relative z-10">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black ${idx === 0 ? 'bg-[#C9A84C] text-black' : 'bg-white/10 text-white light:text-gray-900'}`}>
                          {idx + 1}
                        </div>
                        <div>
                          <p className="font-black text-lg uppercase tracking-wider">{block.block}</p>
                          <p className="text-[9px] font-bold text-white light:text-gray-900/40 uppercase tracking-widest">
                            {block.participants} participants
                          </p>
                        </div>
                      </div>
                      <div className="text-right relative z-10">
                        <p className={`font-black text-2xl ${idx === 0 ? 'text-[#C9A84C]' : 'text-white light:text-gray-900'}`}>
            ...
            <p className="text-center text-[10px] font-bold text-white light:text-gray-900/30 uppercase tracking-widest pt-4">
