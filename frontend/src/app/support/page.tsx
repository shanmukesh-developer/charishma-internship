...
export default function SupportPage() {
  ...
  return (
    <main className="min-h-screen bg-[#020203] text-white light:text-gray-900 p-4 md:p-8 pb-32 overflow-x-hidden relative">
      ...
      <div className="relative z-10 max-w-4xl mx-auto mt-6">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
             ...
             <h1 className="text-2xl font-black uppercase tracking-[0.3em] text-white light:text-gray-900">Support Center</h1>
          </div>
          <button 
        ...
        {showNewTicketForm && (
           <form onSubmit={handleSubmit} className="glass-card-extreme p-6 md:p-8 rounded-3xl border border-primary-yellow/20 mb-10 animate-fade-in relative overflow-hidden">
             ...
             <div className="space-y-6 relative z-10">
               <div>
                 <label className="text-[10px] text-white light:text-gray-900/50 uppercase tracking-widest font-bold ml-2">Issue Subject</label>
                 <input 
                    type="text" 
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="text-[10px] text-white light:text-gray-900/50 uppercase tracking-widest font-bold ml-2">Priority Level</label>
                   <select 
               ...
               <div>
                 <label className="text-[10px] text-white light:text-gray-900/50 uppercase tracking-widest font-bold ml-2">Detailed Description</label>
                 <textarea 
               ...
               <button 
                  ...
                  className="w-full h-14 bg-primary-yellow text-black font-black uppercase tracking-widest text-xs rounded-2xl hover:brightness-110 active:scale-[0.98] transition-all disabled:"
               >
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-white light:text-gray-900/40 mb-4 ml-2 border-b border-white/10 pb-2">Your Tickets</h3>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/5 border-dashed">
              ...
              <p className="text-white light:text-gray-900/40 text-sm font-bold uppercase tracking-widest">No support tickets found</p>
            </div>
          ) : (
            tickets.map(ticket => (
              <div key={ticket.id} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary-yellow/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group">
                 <div>
                    <div className="flex items-center gap-3 mb-2">
                       ...
                       <span className={`px-2 py-1 rounded-[6px] text-[8px] font-black uppercase tracking-widest bg-white/10 text-white light:text-gray-900/60`}>
                         {ticket.priority}
                       </span>
                       <span className="text-[10px] text-white light:text-gray-900/40 font-mono">#{ticket.id.slice(0,8)}</span>
                    </div>
                    <h4 className="text-base font-bold text-white light:text-gray-900 group-hover:text-primary-yellow transition-colors">{ticket.subject}</h4>
                    <p className="text-xs text-white light:text-gray-900/60 mt-2 line-clamp-2">{ticket.description}</p>
                 </div>
                 ...
                 {ticket.adminResponse && (
                    <div className="md:w-1/3 w-full bg-black/40 border border-white/5 p-4 rounded-xl">
                       ...
                       <p className="text-xs text-white light:text-gray-900/80 italic">{ticket.adminResponse}</p>
                    </div>
                 )}
              </div>
            ))
