...
const portals = ['admin-dashboard/src', 'delivery-app/src', 'restaurant-portal/src'];
let filesModified = 0;

portals.forEach(portal => {
  const files = walk(path.resolve(__dirname, '..', portal));
  files.forEach(file => {
    ...
    // 5. Fix TS condition error where if (!'cookie-managed') was introduced
    content = content.replace(/if\s*\(!'cookie-managed'\)/g, "if (false)");
...
console.log(`Modified ${filesModified} files in other portals.`);
